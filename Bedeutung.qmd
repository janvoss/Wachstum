...
Wachstumsrate zwischen 2 Perioden
...
- In en 20 Jahren akkumuliert sich in Land 2 ein zusätzliches BIP das höher ist als die durchschnittliche Jahresleistung in Land 1
...
growth <- df %>% filter(Entity %in% selected_counries) %>% 
  ...
  mutate(Wachstumsraten=Wachstumsraten*100,
         Verdopplungszeit= log(2)/Wachstumsraten)
...
growth <- df %>% group_by(Fall) %>%
  ...
  mutate(Wachstumsraten=round(Wachstumsraten*100))
